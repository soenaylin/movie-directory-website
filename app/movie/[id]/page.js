...
export default async function Movie({ params }) {
  ...
  return (
    <>
      ...
      <div>
        <img className="w-full" src={backdrop + movie.backdrop_path} />
      </div>
      <h3 className="my-4 border-b pb-2">Overview</h3>
      ...
      <div className="flex flex-wrap gap-4 justify-evenly">
        {credits.cast.map((person) => {
          return (
            <div
              key={person.id}
              className="w-[185px] text-center flex flex-col gap-2 border jsutify-between"
            >
              {person.profile_path ? (
